...
make_helper(lui) {
    ...
    sprintf(result_buf, "lui   %s,   0x%04x", REG_NAME(op_dest->reg), op_src2->imm);
}
...
make_helper(ori) {
    ...
    sprintf(result_buf, "ori   %s,   %s,   0x%04x", REG_NAME(op_dest->reg), REG_NAME(op_src1->reg), op_src2->imm);
}
